...
def loop_capturer_dynamic_attributes(start_axis, step, end_axis, tag,
    ...
    for i in drange(start_axis[0], end_axis[0], step):
        for j in drange(start_axis[1], end_axis[1], step):
            ...
            time.sleep(0.02)
            if mode == 'RGB':
                exportImages(ce.toFSPath('images/sVienna4.0'), view, Tag=tag+'_'+str(counter))
            elif mode == 'GT':
                lightSettings = ce.getLighting()
                ...
                ce.setLighting(lightSettings)
                ce.waitForUIIdle()
                exportGroundtruths(ce.toFSPath('images/sVienna4.0'), view, Tag=tag+'_'+str(counter))
            elif mode == 'GT2':
                lightSettings = ce.getLighting()
                lightSettings.setSolarElevationAngle(90)
                lightSettings.setSolarIntensity(1)
                ce.setLighting(lightSettings)
                exportGroundtruths2(ce.toFSPath('images/sVienna4.0'), view, Tag=tag+'_'+str(counter)) #break
...
if __name__ == '__main__':
    ...
    loop_capturer_dynamic_attributes(start_axis=(-1535, -1362), step=400, end_axis=(1685, 1274),
                                     tag='International_City_1', adjust_list = ['la', 'ca', 'li'],
